...
import { addDishToCart, udpateDishInCart } from "api";
...
class DishSidePage extends React.Component {
  ...
  handleApply = () => {
    ...
    if (updateDish) {
      udpateDishInCart(dish, dishToCart);
    } else {
      addDishToCart(dishToCart);
    }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.dishToShow.id !== this.state.id) {
      ...
      if (nextProps.updateDish) {
        const { dishToShow } = nextProps;
        this.setState({
          dish: dishToShow,
          sizes: this.transformDishSizes(Sizes),
          size_id: dishToShow.size_id,
          addlProducts: dishToShow.addlProducts,
          count: dishToShow.count,
          updateDish: true
        });
      } else {
        this.setState({
  ...
  render() {
    ...
    const applyBtnName = updateDish ? "Update" : "Add to cart";
