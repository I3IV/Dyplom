...
import { calendarStore, authStore } from "store";
...
export const getEvents = () =>
  getScheduleItemsByCustId(authStore.user.id).then(
    async scheduleItems =>
      await Promise.all(
  ...
  createScheduleItem({
    ...
    if (event.dishes) {
      event.dishes.forEach(async dish => {
        await createDishesInScheduleItem({
              Dish_id: dish.id,
              ScheduleItem_id: si.id,
              Ammount: dish.count,
