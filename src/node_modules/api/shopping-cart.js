...
export const setCart = value => {
  ...
  let cartValue = cartWithPromocode ? setDiscount(cartWithPromocode) : value;
  cartValue = setRestaurantOrNot(cartValue);
...
const setDiscount = cart => {
  ...
  cart.promoCode.dishes_in_menus.forEach(({ dishes_in_promotion: promo }) => {
    if (R.includes(promo.MenuDish_id, R.map(R.prop("id"), cart.dishes)))
      extract +=
        (R.find(R.propEq("id", promo.MenuDish_id), cart.dishes).totalPrice *
          promo.DiscountValue) /
        100;
  });
  return R.assoc("promoPrice", cart.totalPrice - extract, cart);
};
const setRestaurantOrNot = cartValue => {
  const firstRest_id = !R.isEmpty(cartValue.dishes)
    ? cartValue.dishes[0].Restaurant_id
    : undefined;
  const Restaurant_id =
    firstRest_id &&
    R.all(R.propEq("Restaurant_id", cartValue.dishes[0].Restaurant_id))(
      cartValue.dishes
    )
      ? firstRest_id
      : undefined;

  cartValue.Restaurant_id = Restaurant_id;
  return R.ifElse(
    R.propIs(Number, "Restaurant_id"),
    R.assoc("Restaurant_id", Restaurant_id),
    R.dissoc("Restaurant_id")
  )(cartValue);
};
