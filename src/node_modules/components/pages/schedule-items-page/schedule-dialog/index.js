...
export default class ScheduleDialog extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.dialogOpen !== this.state.open) {
      ...
      const c_id = nextProps.event.category_id;
      if (r_id) this.updateCategoriesSelect(r_id);
      if (c_id) this.updateDishesSelect(c_id);
  ...
  updateSelectedDish(dish_id) {
    if (R.isNil(dish_id) || R.isEmpty(dish_id)) {
      this.setState({ SelectedDish: undefined });
      return;
    }
    this.setState({ loading: true });
    getDish(dish_id).then(dish => {
      ...
      this.setState({
        SelectedDish: {
          ...dish,
          id: dish.dish_sizes[0].DishInMenuId,
          RestaurantDish_id: dish.id,
          addlProducts: [],
          size: dish.dish_sizes[0].DishSizeName,
  ...
  onRestaurantChange = ({ value, name }) => {
    this.updateCategoriesSelect(value);
    const t = R.find(R.propEq("value", value), this.state.SelectRestaurant);
  ...
  onDishSizeChange = size_id => {
    ...
    const size = dish_sizes[sizeIdx].DishSizeName;
    this.setState({
      SelectedDish: {
        ...this.state.SelectedDish,
        id: dish_sizes[sizeIdx].DishInMenuId,
  ...
  deleteProduct = product => {
    const addlProducts = R.reject(
      ...
      this.state.SelectedDish.addlProducts
    );
    this.setState(
      R.assocPath(["SelectedDish", "addlProducts"], addlProducts, this.state)
    );
  };
  toggleProduct = product => {
    const oldAddl = this.state.SelectedDish.addlProducts;
  ...
  onDishAdd = () => {
    const { id, addlProducts } = this.state.SelectedDish;
  ...
  onDishEditSave = () => {
    const { event, SelectedDish } = this.state;
    const { dishes } = event;
    const {
      id,
      SelectedSize,
  ...
  render() {
    const { editMode, event, SelectedDish } = this.state;
    const {
      restaurant_id,
      category_id,
      start,
    ...
    const SelectDish = () =>
      !editMode && (
        <SelectControl
          ...
          name="dish"
          value={SelectedDish ? SelectedDish.RestaurantDish_id : 0}
