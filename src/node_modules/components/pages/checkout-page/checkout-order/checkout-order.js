...
import { createOrderInRestaurant } from "api";
import { CardNumber } from "components/pages/checkout-page/add-card/inputMasks";
...
class Order extends Component {
  ...
  onAddressEntered = address => {
    const { SelectAddresses, address_id } = this.state;

    const toArr = val => R.map(R.trim, R.split(",", val));
    const addr = SelectAddresses
      ? R.find(a => R.equals(toArr(a.label), toArr(address)), SelectAddresses)
      : undefined;
  ...
  handleApply = () => {
    const { comment, orderDateTime, phone, location, address_id } = this.state;
